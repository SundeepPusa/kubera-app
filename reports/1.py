...
CREATE TABLE IF NOT EXISTS {EOD_SCHEMA}.nse_bulk_deals (
  ...
  CONSTRAINT pk_nse_bulk_deals PRIMARY KEY
...
def candidate_csv_urls(trade_date: dt.date) -> List[str]:
    ...
    return [
        f"https://archives.nseindia.com/content/equities/cm_bulkdeals_{ymd}.csv",
        f"https://nsearchives.nseindia.com/content/equities/cm_bulkdeals_{ymd}.csv",
    ]
        ...
        try:
            ...
            if "application/json" in ct:
                ...
                if isinstance(j, dict):
                    data_bulk = j.get("data")
                    if isinstance(data_bulk, dict):
                        for key in ("downloadLink", "path", "url", "uri"):
                            if data_bulk.get(key):
                                urls.append(_abs(data_bulk[key]))
...
def fetch_csv_bytes(d: dt.date, timeout: int, retries: int,
    ...
    raise RuntimeError("Failed to fetch a valid bulk Deals CSV from both archives-json and fallback URLs.")
...
def replace_day_insert(schema: str, trade_date: dt.date, rows: List[Dict[str, Any]], source_file: str,
    ...
    """
    Atomically REPLACE all rows for `trade_date` in {schema}.nse_bulk_deals with `rows`.
    ...
    with pg_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE TEMP TABLE _tmp_bulk_deals(
                  trade_date date,
                  symbol text,
                  security_name text,
                  client_name text,
